...
const CATEGORY_SLUG = "gold-plated-bangles";

export default function GoldPlatedBangles() {
  ...
  useEffect(() => {
    const loadProducts = async () => {
      try {
        setLoading(true);
        const data = await fetchProducts(CATEGORY_SLUG, undefined, 1, 100);
        setProducts(data?.products || []);
      } catch (error) {
        console.error("Failed to fetch gold plated bangles:", error);
      } finally {
        setLoading(false);
      }
    };
      ...
      ${currentPage >= totalPages - 1 ? "opacity-40 cursor-not-allowed" : "hover:bg-[#e2dbd3]"}`}
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-2 md:gap-8">
          {loading ? (
            ...
            <p className="col-span-4 text-center">No gold plated bangles.</p>
          ) : (
